...
- **Classification is keyword rules** by default. `rule_classifier.py::RULES` is an ordered dict; first category whose keyword (matched with `\b…\b` word boundaries against `"{title} {folder}"` — *not* the raw URL, to avoid false positives like `s3`/`rag` inside URLs) appears wins. If none match, `DOMAIN_RULES` maps the URL host to a category (e.g. `youtube.com→entretenimiento`); else `"desconocido"`. Order matters — earlier categories take precedence.
- **Optional AI layer (opt-in, additive).** `ai_classifier.py` calls OpenAI (`gpt-4.1-mini` by default) only when `--use-ai` / `use_ai=True` *and* `OPENAI_API_KEY` is set. It is purely additive: the local rule category/score/action/order are the always-present fallback, and the AI result is attached to `BookmarkAnalysis.ai` (an `AIBookmarkClassification`) without overwriting local fields. Any failure (no key, timeout, bad JSON) silently falls back to local. Config: `OPENAI_MODEL`, `AI_MAX_BOOKMARKS`, `AI_TIMEOUT_SECONDS`. Tests mock the provider — never call OpenAI for real in tests.
